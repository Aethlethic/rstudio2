...
corrplot(corr, method = 'color', col(200), addCoef.col = 'black',
          type = 'upper',tl.cex = 0.75, number.cex = 0.75)

 # 자기 자신을 제외한 변수와도 높은 상관관계가 보임임
```
...
pcr_model <- pcr(Appliances ~ ., dat = train, validation='CV')
...
plot(pcr_model, 'loadings', comps=1:6,
     legendpos='topright')
...
#모델 수립
plsr_model <- plsr(Appliances ~ ., dat = train, validation='CV')
...
plot(plsr_model, 'loadings', comps=1:7,
     legendpos='topright')
